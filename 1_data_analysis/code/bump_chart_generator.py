"""
Bump Chart Generator for Domain Flow Analysis

This module creates bump charts showing the flow of domains through three layers:
...
class BumpChartGenerator:
    """Generate bump charts for domain flow analysis across three layers."""
    ...
    def load_region_data(self, region: str) -> tuple:
        """
        Load data for a specific region across all three layers.

        Args:
            region (str): Region code ('UK', 'US', 'EU'/'Europe')

        Returns:
            tuple: (layer1_data, layer2_data, layer3_data)
        """
        ...
        return layer1_data, layer2_data, layer3_data

    def get_unique_domains_and_colors(
        self, layer1: pd.DataFrame, layer2: pd.DataFrame, layer3: pd.DataFrame
    ) -> dict:
        """
        Get all unique domains from the top N domains across all three layers WITHIN A SINGLE REGION
    ...
    def process_layer_data(
        ...
        Args:
            ...
            count_col (str): Name of the count column ('frequency' or 'total_count')
    ...
    def create_flow_data(
        ...
        Args:
            layer1, layer2, layer3 (pd.DataFrame): Processed layer data
    ...
    def plot_bump_chart(
        ...
        layer_names = ["Search Query", "Web Search API", "GPT-5 Final Response"]
        ...
        for _, row in visible_domains.iterrows():
            ...
            # Make lines 3x thicker (was 3, now 9) and more transparent
            if len(positions) > 1:
                ax.plot(
            ...
            for pos, rank, count in zip(positions, ranks, counts):
                ...
                # Add count text inside bubble with doubled font size
                ax.text(
                    pos,
                    rank,
                    str(int(count)),
                ...
                ax.text(
                    pos,
                    rank + 0.32,  # Position below bubble (reduced spacing)
                    domain,
                    ha="center",
                    va="top",
                    fontsize=30,
        ...
        ax.text(
            0.02,
            0.98,  # Increased to add more space above plot
            f"{region}",
        ...
        plt.tight_layout(pad=0, rect=[0, 0.0, 1, 1])  # rect=[left, bottom, right, top]
        return fig
    ...
    def generate_chart_for_region(
        ...
        """
        print(f"Generating bump chart for {region_display}...")

        try:
            # Load data
            layer1, layer2, layer3 = self.load_region_data(region_code)
            ...
            domain_colors = self.get_unique_domains_and_colors(
                layer1_processed, layer2_processed, layer3_processed
            )
            ...
            output_file = self.output_dir / f"bump_chart_{filename_region}.png"
            fig.savefig(
                output_file,
                dpi=300,
                bbox_inches="tight",
...
def main():
    """Main function to generate all bump charts."""
    ...
    generator = BumpChartGenerator(data_dir, output_dir)
    generator.generate_all_charts(top_n=10)

    print("\nAll bump charts generated successfully!")
